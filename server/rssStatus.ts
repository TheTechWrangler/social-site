...
const itemCount = (db.prepare("SELECT COUNT(*) as c FROM external_items WHERE provider = 'rss'").get() as any).c;
const latest = db.prepare("SELECT published_at, title FROM external_items WHERE provider = 'rss' ORDER BY published_at DESC LIMIT 1").get() as any;
const nullFetch = sources.filter(s => !s.last_fetched_at);

console.log(`  RSS sources:     ${sources.length} (${active.length} active)`);
const podcastCount = (db.prepare("SELECT COUNT(*) as c FROM external_items WHERE provider = 'rss' AND item_kind = 'podcast'").get() as any).c;
const articleCount = (db.prepare("SELECT COUNT(*) as c FROM external_items WHERE provider = 'rss' AND item_kind = 'article'").get() as any).c;
console.log(`  RSS items:       ${itemCount} (${articleCount} articles, ${podcastCount} podcasts)`);
