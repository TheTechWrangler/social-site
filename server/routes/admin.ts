...
router.get('/stats', requireAuth, requireAdmin, (_req, res) => {
  ...
  res.json({
    ...
    activeRssSources: (db.prepare("SELECT COUNT(*) as c FROM external_sources WHERE provider = 'rss' AND source_kind = 'rss' AND is_active = 1 AND tombstoned_at IS NULL").get() as any).c,
    rssItemCount: (db.prepare("SELECT COUNT(*) as c FROM external_items WHERE provider = 'rss'").get() as any).c,
